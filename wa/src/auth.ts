// Cuentas de la bandeja: contraseñas, sesiones por cookie y freno a fuerza bruta.
//
...
export const COOKIE = 'tratto_sesion';
...
export function sessionCookie(env: Env, token: string | null): string {
  // Path=/inbox: la cookie no viaja al webhook ni a /admin. Secure solo en prod,
  // porque en `wrangler dev` la página se abre por http.
  const base = `${COOKIE}=${token ? encodeURIComponent(token) : ''}; Path=/inbox; HttpOnly; SameSite=Lax`;
